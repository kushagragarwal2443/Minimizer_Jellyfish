    ...
    while(true) {
      ...
      do {
        ...
        if(code >= 0) {
          ...
          // std::cout << "BEFORE: " << kmer_int[0] << "\t" << code << std::endl;//Kushagra's addition
          kmer_int[0] = (kmer_int[0] << 2 | code) & mask1; // forward k-mer // Souvadra's addition
          // std::cout << "AFTER: " << kmer_int[0] << std::endl;//Kushagra's addition
      ...
      if(filled_ >= m_.k())
      {
        ...
        // std::cout << "KMER_INT: " << kmer_int[0] << std::endl; //Kushagra's addition

        m_.set_rid(read_number); // Souvadra's addition
        m_.set_kmer_int(kmer_int[0]); // Souvadra's addition

        // std::cout << kmer_int[0] << std::endl; //Kushagra's addition
        // std::cout << std::endl; //Kushagra's addition
