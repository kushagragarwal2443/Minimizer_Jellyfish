...
static inline uint64_t hash64(uint64_t key, uint64_t mask) {
  ...
  // std::cout << "MASK: " << mask << std::endl; //Kushagra's addition
  ...
  // std::cout << "HASHED KMER_INT: " << key << std::endl; //Kushagra's addition
...
public:
  ...
  */
  ...
  void minimizer_helper(uint64_t kmer_int, int z) {
    ...
    // std::cout << "L: " << l << std::endl; //Kushagra's addition
    // std::cout << "BUFPOS: " << buf_pos << std::endl; //Kushagra's addition
    // std::cout << "MIN.X: " << min.x << std::endl; //Kushagra's addition

    if (l >= k && kmer_span < 256) {

      // std::cout << "KMER_INT RECEIVED: " << kmer_int << std::endl; //Kushagra's addition
      info.x = hash64(kmer_int, mask) << 8 | kmer_span;
      // std::cout << "KMER_SPAN: " << kmer_span << std::endl; //Kushagra's addition
      // std::cout << "INFO.X: " << info.x << std::endl; //Kushagra's addition
    ...
    if (l == w + k - 1 && min.x != UINT64_MAX) { // special case for the first window -because identical k-mers are not stored yet
      ...
      // std::cout << "GOING INSIDE FIRST IF CONDITION" << std::endl; //Kushagra's addition

      for (j = buf_pos + 1; j < w; ++j)
      {
        if (min.x == buf[j].x && buf[j].y != min.y) 
        {
          // std::cout << "FIRST FOR LOOP, Pushing back j: " << j << std::endl; //Kushagra's addition
      ...
      for (j = 0; j < buf_pos; ++j)
      {
        if (min.x == buf[j].x && buf[j].y != min.y) 
        {
          // std::cout << "SECOND FOR LOOP, Pushing back j: " << j << std::endl; //Kushagra's addition
          return_mer.push_back(j);
        }
      }
		}
    ...
    if (info.x <= min.x) { // a new minimum; then write the old min
      ...
      // std::cout << "GOING INSIDE SECOND IF CONDITION" << std::endl; //Kushagra's addition

      new_min = true;
      if (l >= w + k && min.x != UINT64_MAX) 
      {
        // std::cout << "Pushing back -1 " << std::endl; //Kushagra's addition
      ...
      // std::cout << "GOING INSIDE ELSE IF CONDITION" << std::endl; //Kushagra's addition
...
public:
  ...
  virtual void start(int thid) {
    ...
    minimizer_factory mmf(mers->k(),mers->k()); // w value hardcoded, NEET TO CHANGE
     ...
     case COUNT:
      ...
      for (; mers; ++mers) {
        if((*filter_)(*mers)) {
          ...
          // std::cout << "MERS: " << mers->to_str() << std::endl; 
