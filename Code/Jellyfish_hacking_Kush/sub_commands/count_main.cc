...
#include <typeinfo> //Kushagra addition
...
// #define nostar_mers_type jellyfish::mer_iterator::std::input_iterator_tag::
...
public:
  ...
  virtual void start(int thid) {
    ...
    int *popat;
    popat = &ramu;
    long unsigned int shamu = 0;
    long unsigned int *babita;
    babita = &shamu;
     ...
     case COUNT:
      ...
      for (; mers; ++mers) {
        if((*filter_)(*mers)) {
          ...
          std::cout << "mers: " << typeid(mers).name() << std::endl;
          std::cout << "*mers: "<< typeid(*mers).name() << std::endl;
          std::cout << "int: "<< typeid(ramu).name() << std::endl;
          std::cout << "Pointer to int: "<< typeid(popat).name() << std::endl;
          std::cout << "Deferenced Pointer to int: "<< typeid(*popat).name() << std::endl;
          std::cout << "long unsigned int: "<< typeid(shamu).name() << std::endl;
          std::cout << "Pointer to long unsigned int: "<< typeid(babita).name() << std::endl;
          std::cout << "Deferenced Pointer to long unsigned int: "<< typeid(*babita).name() << std::endl;
          std::cout << std::endl;
