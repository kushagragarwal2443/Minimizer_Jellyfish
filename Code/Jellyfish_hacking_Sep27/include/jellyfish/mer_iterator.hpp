...
public:
  ...
  mer_iterator& operator++() {
    while(true) {
      if (cseq_ == (*job_)->start) std::cout << cseq_ << " | Mer iterator line 59; Job number is: "<< job_number << std::endl; // Souvadra's addition
      while(cseq_ == (*job_)->end) {
        ...
        cseq_   = (*job_)->start;
        std::cout << cseq_ << " | Mer iterator line 70; Job number is: "<< job_number << std::endl; // Souvadra's addition
