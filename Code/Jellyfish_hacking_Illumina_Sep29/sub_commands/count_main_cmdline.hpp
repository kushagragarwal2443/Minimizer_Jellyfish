...
public:
  ...
  bool                           mer_len_given;
  uint32_t                       window_len_arg; // Kushagra Added
  bool                           window_len_given; // Kushagra Added
  uint32_t                       read_len_arg; // Kushagra Added
  bool                           read_len_given; // Kushagra Added
  uint64_t                       size_arg;
  bool                           size_given;
  ...
  count_main_cmdline() :
    ...
    window_len_arg(0), window_len_given(false), // Kushagra Added
    read_len_arg(0), read_len_given(false),     // Kushagra Added
  ...
  void parse(int argc, char* argv[]) {
    static struct option long_options[] = {
      {"mer-len", 1, 0, 'm'},
      {"window-len", 1, 0, 'w'},  // Kushagra Added
      {"read-len", 1, 0, 'r'},    // Kushagra Added
      {"size", 1, 0, 's'},
      {"threads", 1, 0, 't'},
      {"sam", 1, 0, SAM_OPT},
      {"Files", 1, 0, 'F'},
    ...
    static const char *short_options = "hVm:w:r:s:t:F:g:G:S:o:c:CQ:p:L:U:"; // Kushagra Added w and r in this
...
#define CHECK_ERR(type,val,which) if(!err.empty()) { ::std::cerr << "Invalid " #type " '" << val << "' for [" which "]: " << err << "\n"; exit(1); }
    while(true) {
      ...
      case 'w':
        window_len_given = true;
        window_len_arg = conv_uint<uint32_t>((const char*)optarg, err, false);
        CHECK_ERR(uint32_t, optarg, "-w, --window-len=uint32")
        break;

      case 'r':
        read_len_given = true;
        read_len_arg = conv_uint<uint32_t>((const char*)optarg, err, false);
        CHECK_ERR(uint32_t, optarg, "-r, --read-len=uint32")
        break;
    ...
    // Kushagra addition starts here
    if(!window_len_given)
      error("[-w, --window-len=uint32] required switch");
    if(!read_len_given)
      error("[-r, --read-len=uint32] required switch");
    //Kushagra addition ends here
  ...
  // Kushagra addition in this as well
  static const char * help() { return
    "Count k-mers in fasta or fastq files\n\n"
    "Options (default value in (), *required):\n"
    " -m, --mer-len=uint32                    *Length of mer\n"
    " -w, --window-len=uint32                 *Length of window for minimizer selection\n"
    " -r, --read-len=uint32                   *Number of bases in each read of the input data\n"
...
#define PACKAGE_VERSION "0.0.0"
  ...
  void dump(::std::ostream &os = std::cout) {
    ...
    // Kushagra addition starts here
    os << "window_len_given:" << window_len_given << " window_len_arg:" << window_len_arg << "\n";
    os << "read_len_given:" << read_len_given << " read_len_arg:" << read_len_arg << "\n";
    // Kushagra addtion ends here
