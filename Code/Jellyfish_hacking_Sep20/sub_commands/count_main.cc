...
public:
  ...
  // THIS IS KUSHAGRA'S VERSION OF ROBUST WINNOWING AS ON SEP 4
    ...
    if (info.x <= min.x && l<= w+k-1 && info.y == min.y) // get right most in first window
    {
      new_min = true;
      min = info;
      min_pos = buf_pos;
    ...
    if (info.x < min.x && l>= w+k && info.y == min.y)
    {
      new_min = true;
      if (min.x != UINT64_MAX)
      {
    ...
    { 
      ...
      for (j = buf_pos + 1; j < w; ++j) // the two loops are necessary when there are identical k-mers
        if (buf[j].x <= min.x && buf[j].y == min.y) min = buf[j], min_pos = j; //  >= is important s.t. min is always the closest k-mer
      for (j = 0; j <= buf_pos; ++j)
        if (buf[j].x <= min.x && buf[j].y == min.y) min = buf[j], min_pos = j; 
    }
    #endif
     ...
     case COUNT:
      ...
      for (; mers; ++mers) {
        if((*filter_)(*mers)) {
          ...
          // std::cout << mers->to_str() << " " << mers->get_skip() << std::endl;
