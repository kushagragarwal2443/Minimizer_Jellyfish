...
static inline uint64_t hash64(uint64_t key, uint64_t mask) {
  ...
  // std::cout << "WITHIN THE MASKING FUNCTION\n"; //Kush additions
  // std::cout << "KEY is: " << key << std::endl; //Kush additions
  // std::cout << "MASK is: " << mask << std::endl; //Kush additions
  ...
  // std::cout << "FINAL KEY is: " << key << std::endl; //Kush additions
...
private:
  ...
  std::vector<star_mers_type> buf_mer; 
  star_mers_type min_mer;
public:
  ...
  minimizer_factory(int k, int w) {
    ...
    // KUSHAGRA: Filling 255 till w*16 bytes from buf
    // KUSHAGRA: Why 16? Takes 16 bytes at max to score a kmer?
    // KUSHAGRA: Would mean that it is storing all the kmers in a window in buf
    memset(buf, 0xff, w * 16);

    // KUSHAGRA: Do we need this anymore, where are we using it???
    memset(&tq, 0, sizeof(tiny_queue_t));
  }

  // KUSHAGRA: I do not understand this function
  ...
  void minimizer_helper(star_mers_type mer, int c) {
    ...
    { 
      int z;
      // KUSHAGRA: Removed is_hpc assuming we get filtered kmers within this function

      kmer_span = l + 1 < k? l + 1 : k; // KUSHAGRA: This is not correct, l starts with 0
      // KUSHAGRA: as it was initialized in private variables, but we already get kmers in this func 
      // KUSHAGRA: std::cout << "Printing kmer_span, l and k: " << kmer_span << "\t" << l << "\t" << k << std::endl;
      ...
      // KUSHAGRA: I dont think that is the correct way to proceed, should ask sir
      ...
      {
        // KUSHAGRA: kmer_span remains ==k in our case, we should remove this from info.x and just use the hash value returned
        // KUSHAGRA: std::cout << kmer_span << std::endl;
        info.x = hash64(kmer[z], mask) << 8 | kmer_span;
			  info.y = (uint64_t)rid<<32 | (uint32_t)i<<1 | z;
        // KUSHAGRA:
        // std::cout << "INFO.X: " << info.x << std::endl;
        // std::cout << "INFO.Y: " << info.y << std::endl;
        info_mer = mer; // Souvadra's addition
      }
      // // KUSHAGRA: This is where the code would end up reaching for the first k-1 chars
      // else
      // {
      //   // std::cout << "HELLO I AM IN THE ELSE PART OF THE l>=k IF CONDITION\n";
      // }

    } 
    else 
    {
      // KUSHAGRA: Can we just remove l, tq and kmer_span i dont think we will be using it
      // Therefore completely remove this else condition
  ...
  void select_minimizer(star_mers_type mer) {
    if (buf_mer.empty()) 
    {
      ...
      for (i = l = 0; i < (int)str.length(); ++i) 
      {
        ...
        // KUSHAGRA addition: std::cout << "BUF_MER EMPTY\t" << mer.to_str() << "\t" << c << std::endl;
    ...
    else 
    {
      ...
      // KUSHAGRA addition: std::cout << "BUF_MER FULL\t" << mer.to_str() << "\t" << c << std::endl;
      minimizer_helper(mer,c);
    }
    // signal = true;
  }
...
public:
  ...
  virtual void start(int thid) {
    ...
    minimizer_factory mmf(4,4); // k and w value hardcoded, NEED TO CHANGE
    switch(op_) {
     case COUNT:
      #if 1
      // KUSHAGRA addition:
      ...
      for (; mers; ++mers) 
      {
        if((*filter_)(*mers)) {
          ...
          // {
          //   mer_counter = 0;
          //   for (mer_i = 0; mer_i < (int)mer_str.length() - 1; ++mer_i) 
          //   {
          //     char mer_c = mer_str[mer_i];
          //     char mer_cp = mer_prev[mer_i+1];
          //     if(mer_c == mer_cp){mer_counter += 1;}              
          //   }
          //   if(mer_counter != (int)mer_str.length() - 1)
          //   {
          //     std::cout << "READ OVER\n";
          //     mer_flag = 0;
          //   }
