...
public:
  ...
  void minimizer_helper(uint64_t kmer_int, int z) {
    ...
    if (l == w + k - 1 && min.x != UINT64_MAX) { // special case for the first window -because identical k-mers are not stored yet
      for (j = buf_pos + 1; j < w; ++j)
      {
        ...
        {
          std::cout << "LOOPING IN FIRST WINDOW " << buf[j].x << " " << std::endl;
  ...
  void select_minimizer(uint64_t kmer_int, uint32_t rid, int strand) {
    if (this->rid != rid) {
        ...
        if (rid != 1) 
        {
          // std::cout << "PUSHING READ CHANGE MINIMIZER" << std::endl;
          // std::cout << min.x << " " << min.y << " " << min_pos << " " << buf_pos << " " << l << std::endl;
          // std::cout << std::endl;
          std::cout << "PUSHING READ CHANGE MINMIZER " << min.x << " " << std::endl;
    ...
    switch(op_) {
     case COUNT:
        ...
        if((*filter_)(*mers)) {
          ...
          while (!mmf.return_mer.empty()) {
            ...
            if (mer_pos == -1) {
              ...
              {
                std::cout << "IF1: " << min_mer.to_str() << std::endl; // Souvadra's addition
                ary_.add(min_mer, 1);
              }
              //if (min_initialized == 1) std::cout << "line 316: " << min_mer.to_str() << std::endl;
            } 
            else {
              std::cout << "ELSE1: " << buf_mer_2[mer_pos].to_str() << std::endl; // Souvadra's addition
      ...
      if (true) { // souvadra's addition
        ...
        if (min_initialized == 1) 
        {
          // std::cout << "PUSHING END OF FILE MINIMIZER" << std::endl;
          // std::cout << min_mer << std::endl;
          // std::cout << std::endl;
          std::cout << "END OF FILE MINIMIZER: " << min_mer.to_str() << std::endl;
          ary_.add(min_mer, 1); 
